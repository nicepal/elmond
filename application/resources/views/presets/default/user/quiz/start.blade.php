                                        ...
                                        <div class="col-sm-8 my-3">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" value="{{ $index }}"
                                                    id="flexCheckChecked_{{ $index }}_{{ $item->id }}" name="answer[]" data-quiz_id={{ $quiz->id }}
                                                    data-question_id={{ $item->id }} onchange="checkedCheckBox(this)">
                                                <label class="form-check-label" for="flexCheckChecked_{{ $index }}_{{ $item->id }}">
                                                    {{ $option }}
                                                </label>
                                            </div>
                                        </div>
...
@push('script')
    ...
    <script>
        ...
        <script>
            ...
            // Disable F12, Ctrl+Shift+I, and other inspect shortcuts
            document.addEventListener('keydown', (event) => {
                if (
                    event.key === 'F12' ||
                    (event.ctrlKey && event.shiftKey && (event.key === 'I' || event.key === 'J')) ||
                    (event.ctrlKey && event.key === 'U')||(event.ctrlKey && event.key === 'u')||(event.key === 'i' || event.key === 'j')
                ) {
