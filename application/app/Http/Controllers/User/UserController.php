...
class UserController extends Controller
{
    ...
    public function userData()
    {
        ...
        // Check if user data is already complete and there are courses to enroll
        $isDataComplete = !empty($user->firstname) && 
                         !empty($user->lastname) && 
                         !empty($user->address->address ?? null) && 
                         !empty($user->address->state ?? null) && 
                         !empty($user->address->zip ?? null) && 
                         !empty($user->address->city ?? null);
        
        // Check if there are courses in session for enrollment
        $hasCoursesToEnroll = session()->has('enrollment_courses') && !empty(session('enrollment_courses'));
        
        // If data is complete and there are courses to enroll, redirect to enrollment
        if ($isDataComplete && $hasCoursesToEnroll) {
            // Mark registration as complete
            $user->reg_step = 1;
            $user->save();
            
            $notify[] = ['success', 'Profile already complete. Proceeding to enrollment.'];
            return redirect()->route('user.enroll.multi')->withNotify($notify);
        }
